...
from LiGlobal.tool.downloader.cmd.BaseDownloader import DownloadEngine, BaseDownloaderThread, latlng2tile_wmts, BoundBox


class TDTDownloaderThread(BaseDownloaderThread):
    URL = "http://t%s.tianditu.cn/DataServer?T=img_w&X={x}&Y={y}&L={z}"

    def __init__(self, root_dir, bbox, task_q, logger=None):
        super(TDTDownloaderThread, self).__init__(root_dir, bbox, task_q, logger)
    ...
    def _download(self, x, y, z):
        file_path = '%s/%s/%i/%i/%i.%s' % (self.root_dir, 'tianditu', z, x, y, 'jpg')
...
class TDTDownloadEngine(DownloadEngine):
    ...
    def __init__(self, root_dir, bbox, thread_num, logger):
        super(TDTDownloadEngine, self).__init__(bbox, thread_num, logger)
        self.root_dir = root_dir

    def bbox2xyz(self, z):
        min_x, min_y = latlng2tile_wmts(self.bbox.max_lat, self.bbox.min_lng, z)
        max_x, max_y = latlng2tile_wmts(self.bbox.min_lat, self.bbox.max_lng, z)
    ...
    def run(self):
        try:
            ...
            for i in range(self.thread_num):
                thread = TDTDownloaderThread(self.root_dir, self.bbox, task_q, self.logger)
...
if __name__ == '__main__':
    ...
    try:
        ...
        d = TDTDownloadEngine(root, bbox, 4, logger)
        d.start()
        time.sleep(10000)
