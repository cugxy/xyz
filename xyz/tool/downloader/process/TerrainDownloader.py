...
# 与 Cesium 版本有关
# token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJkZGM5ZDhhNi00MTEzLTQ3YzUtOGQ1Zi0xZjE3MGM0NDYzNWEiLCJpZCI6MjU5LCJpYXQiOjE1MzU3MjQyMjh9.pIEOyZPuF4xHRzAQFnDgZJwgpLXDtS7vGMwha6fCs2g'
...
class TerrainDownloaderThread(BaseDownloaderThread):
    # URL = "https://assets.agi.com/stk-terrain/world/{z}/{x}/{y}.terrain?extensions=octvertexnormals-watermask&v=1.1.0"
    URL = "https://assets.cesium.com/1/{z}/{x}/{y}.terrain?extensions=octvertexnormals-watermask&v=1.1.0"

    def __init__(self, root_dir, bbox, token, task_q, logger=None):
        super(TerrainDownloaderThread, self).__init__(root_dir, bbox, task_q, logger)
        self.token = token
...
class TerrainDownloadEngine(DownloadEngine):
    ...
    def __init__(self, root_dir, bbox, token, thread_num, logger):
        super(TerrainDownloadEngine, self).__init__(bbox, thread_num, logger)
